...
class DeclListNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTable, SymTable tmpSymTable) {
        Iterator<DeclNode> it = myDecls.iterator();
        while (it.hasNext()) {
            ((VarDeclNode) it.next()).nameAnalysis(symTable, tmpSymTable);
        }
    }
...
class FormalsListNode extends ASTnode {
    ...
    public List<String> getType() {
        ...
        try {
            while (it.hasNext()) {
                ret.add(((FormalDeclNode) it.next()).getType());
            }
        } catch (NoSuchElementException ex) {
...
class VarDeclNode extends DeclNode {
    ...
    // for var decl not in a struct definition
    public void nameAnalysis(SymTable symTable) {
    ...
    // try {
    // ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Non-function declared
    // void");
    // if (symTable.lookupLocal(myId.getName()) != null) {
    // ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Multiply declared
    // identifier");
    // }
    // } catch (Exception e) {
    // System.err.println("unexpected EmptySymTableException in
    // VarDeclNode.nameAnalysis()");
    // }
    // }
    // // if type is struct
    // else if (myType instanceof StructNode) {
    // try {
    // Sym tmpSym = symTable.lookupGlobal(myType.getType());
    // boolean hasError = false;
    // if (tmpSym == null) {
    // ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Invalid name of struct
    // type");
    // hasError = true;
    // }
    // if (symTable.lookupLocal(myId.getName()) != null) {
    ...
    // return;
    // }
    // symTable.addDecl(myId.getName(), new StructSym(myType.getType(),
    // ((StructDeclSym) tmpSym).getFields()));
    // } catch (Exception e) {
    // System.err.println("unexpected EmptySymTableException in
    // VarDeclNode.nameAnalysis()");
    // }
    // }
    // // if type is int or bool
    // else {
    // try {
    ...
    // for var decl in a struct definition
    public void nameAnalysis(SymTable symTable, SymTable tmpSymTable) {
        // if type is void
        if (myType.getType() == "void") {
            try {
                ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Non-function declared void");
                if (tmpSymTable.lookupLocal(myId.getName()) != null) {
        ...
        // if type is struct
            ...
            try {
                ...
                tmpSymTable.addDecl(myId.getName(),
                        new StructSym(myType.getType(), ((StructDeclSym) tmpSym).getFields()));
            } catch (Exception e) {
                System.err.println("unexpected EmptySymTableException in VarDeclNode.nameAnalysis()");
            }
        }
        // if type is int or bool
            ...
            try {
                tmpSymTable.addDecl(myId.getName(), new VarSym(myType.getType()));
            } catch (DuplicateSymException e) {
                ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Multiply declared identifier");
            } catch (Exception e) {
                System.err.println("unexpected EmptySymTableException in VarDeclNode.nameAnalysis()");
...
class FnDeclNode extends DeclNode {
    ...
    public void nameAnalysis(SymTable symTable) {
        // add fn decl into current scope
        try {
        ...
        // add new scope and continue name analysis
        symTable.addScope();
...
class FormalDeclNode extends DeclNode {
    ...
    public void nameAnalysis(SymTable symTable) {
        try {
            ...
            System.err.println("unexcepted exception in VarDeclNode.nameAnalysis()");
        }
    }
...
class StructDeclNode extends DeclNode {
    ...
    public void nameAnalysis(SymTable symTable) {
        try {
            SymTable tmpSymTable = new SymTable();
            myDeclList.nameAnalysis(symTable, tmpSymTable);
            StructDeclSym tmpSym = new StructDeclSym(tmpSymTable);
            symTable.addDecl(myId.getName(), tmpSym);
        } catch (DuplicateSymException e) {
            ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Multiply declared identifier");
        } catch (Exception e) {
            System.err.println("unexcepted exception in StructDeclNode.nameAnalysis()");
        }
    }
...
class IdNode extends ExpNode {
    ...
    public void nameAnalysis(SymTable symTable) {
        try {
            ...
            System.err.println("unexpected Exception in IdNode.nameAnalysis()");
        }
    }
...
class DotAccessExpNode extends ExpNode {
    ...
    public void nameAnalysis(SymTable symTable) {
        if (myLoc instanceof IdNode) {
            ...
            Sym tmpSym = ((IdNode) myLoc).getSym();
            if (!(tmpSym instanceof StructSym)) {
                ErrMsg.fatal(((IdNode) myLoc).getLineNum(), ((IdNode) myLoc).getCharNum(),
                        "Dot-access of nonstruct type");
            } else {
                try {
                    if (((StructSym) tmpSym).getFields().lookupGlobal(myId.getName()) == null) {
                        ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Invalid struct field name");
                    } else {
                        myId.nameAnalysis(((StructSym) tmpSym).getFields());
                    }
                } catch (Exception e) {
                    System.err.println("unexpected Exception in DotAccessExpNode.nameAnalysis()");
                }
            }
        } else if (myLoc instanceof DotAccessExpNode) {
            myLoc.nameAnalysis(symTable);
            Sym tmpSym = ((DotAccessExpNode) myLoc).getIdNode().getSym();
            if (!(tmpSym instanceof StructSym)) {
                ErrMsg.fatal(((DotAccessExpNode) myLoc).getIdNode().getLineNum(),
                        ((DotAccessExpNode) myLoc).getIdNode().getCharNum(), "Dot-access of nonstruct type");
            } else {
                try {
                    if (((StructSym) tmpSym).getFields().lookupGlobal(myId.getName()) == null) {
                        ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Invalid struct field name");
